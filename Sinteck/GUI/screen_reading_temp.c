...
void screen_reading_temp(void)
{
	...
	lv_obj_set_style(Tela_Reading_TEMP, &style_fundo); 					// Configura o estilo criado
...
void update_vumeter_temperatura(float temp)
{
	...
	for(x = 0; x <= resul; x++) {
		if(x >= 16) {
			lv_bar_set_style(bar_t[x], LV_BAR_STYLE_BG, &style_indic_bar_vm);
			lv_bar_set_style(bar_t[x], LV_BAR_STYLE_INDIC, &style_indic_bar_vm);
		}
		else if(x > 14 && x < 16) {
			lv_bar_set_style(bar_t[x], LV_BAR_STYLE_BG, &style_indic_bar_am);
			lv_bar_set_style(bar_t[x], LV_BAR_STYLE_INDIC, &style_indic_bar_am);
		}
		else {
