...
extern float f_vpa, f_ipa;
extern uint32_t vpa, ipa;
...
void screen_reading_vpa(void)
{
	...
	lv_obj_set_style(Tela_Reading_VPA, &style_fundo); 					// Configura o estilo criado
	...
	print_vpa_1(f_vpa);
	print_ipa_1(f_ipa);
	update_vumeter_vpa_1(f_vpa);
	update_vumeter_ipa_1(f_ipa);
...
void update_vumeter_vpa_1(float value)
{
	...
	if((float)value !=0) {
		resul = (uint32_t)(value / 0.75);
		if(resul > 21) resul = 20;
	}
	else {
		resul = 0;
	}
...
static void update_screen_vpa(lv_task_t * param)
{
	sprintf(buffer, "%0.1f", f_vpa);
	lv_label_set_text(txt_vpa, buffer);
	update_vumeter_vpa_1(f_vpa);

	sprintf(buffer, "%0.1f", f_ipa);
	lv_label_set_text(txt_ipa, buffer);
	update_vumeter_ipa_1(f_ipa);
}
