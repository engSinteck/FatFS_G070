...
static void btn_stereo(lv_obj_t * btn, lv_event_t event)
{
	if(event == LV_EVENT_RELEASED) {
		...
		if(id == 0) {
			lv_obj_align(bar[0], NULL, LV_ALIGN_IN_TOP_LEFT, 54, 68);
			Cfg_Stereo = 1;
			//HAL_GPIO_WritePin(STEREO_MONO_GPIO_Port, STEREO_MONO_Pin, GPIO_PIN_RESET);
		}
		else if(id == 1) {
			lv_obj_align(bar[0], NULL, LV_ALIGN_IN_TOP_LEFT, 54, 88);
			Cfg_Stereo = 0;
			//HAL_GPIO_WritePin(STEREO_MONO_GPIO_Port, STEREO_MONO_Pin, GPIO_PIN_SET);
		}
		//logI("Cfg_Stereo: %d\n", Cfg_Stereo);
	}
}
...
static void btn_processador(lv_obj_t * btn, lv_event_t event)
{
	if(event == LV_EVENT_RELEASED) {
		...
		if(id == 0) {
			lv_obj_align(bar[1], NULL, LV_ALIGN_IN_TOP_LEFT, 132, 68);
			Cfg_Processador = 1;
			//HAL_GPIO_WritePin(PROCESSOR_SEL_GPIO_Port, PROCESSOR_SEL_Pin, GPIO_PIN_SET);
		}
		else if(id == 1) {
			lv_obj_align(bar[1], NULL, LV_ALIGN_IN_TOP_LEFT, 132, 88);
			Cfg_Processador = 0;
			//HAL_GPIO_WritePin(PROCESSOR_SEL_GPIO_Port, PROCESSOR_SEL_Pin, GPIO_PIN_RESET);
		}
...
static void btn_clipper(lv_obj_t * btn, lv_event_t event)
{
	if(event == LV_EVENT_RELEASED) {
		...
		if(id == 0) {
			lv_obj_align(bar[0], NULL, LV_ALIGN_IN_TOP_LEFT, 54, 68);
			Cfg_Clipper = 1;
			//HAL_GPIO_WritePin(CLIPPER_GPIO_Port, CLIPPER_Pin, GPIO_PIN_SET);
		}
		else if(id == 1) {
			lv_obj_align(bar[0], NULL, LV_ALIGN_IN_TOP_LEFT, 54, 88);
			Cfg_Clipper = 0;
			//HAL_GPIO_WritePin(CLIPPER_GPIO_Port, CLIPPER_Pin, GPIO_PIN_RESET);
		}
		//logI("Cfg_Clipper: %d\n", Cfg_Clipper);
	}
}
